...
MERGE INSTRUCTIONS:
1. Add the Elements class definition shown below to PrintHistoryPage
2. Locate the existing __init__ method in PrintHistoryPage
3. Ensure self.elements = self.Elements() is called in __init__
4. Add the verification methods shown below to the class body
5. Maintain consistent indentation with the existing file

MERGE NOTES:
- Tooltip selectors currently use [role='tooltip'] - update with specific data-testid attributes when available
...
# ═══════════════════════════════════════════════════════════════════════════════
# ADD TO EXISTING Elements CLASS OR CREATE NEW Elements CLASS
# ═══════════════════════════════════════════════════════════════════════════════

class Elements:
    # Existing elements...
    
    # New elements for billing cycle card
    ...
    # TODO: Update with specific data-testid when available
...
def verify_plan_pause_info_not_displayed(self):
    ...
    expect(self.elements.plan_pause_info).not_to_be_visible(timeout=10000)


def verify_plan_pause_info_displayed(self):
    """Verify plan pause information is displayed."""
    expect(self.elements.plan_pause_info).to_be_visible(timeout=30000)
...
def verify_complimentary_pages_value(self, expected_used, expected_total, pause_plan=True):
    ...
    expect(self.elements.complimentary_pages_value).to_contain_text(f"{expected_used} of {expected_total}", timeout=30000)
    if pause_plan:
        expect(self.elements.complimentary_pages_value).to_contain_text("Pause Plan", timeout=30000)


def hover_complimentary_info_icon(self):
    """Hover over complimentary pages info icon to display tooltip."""
    expect(self.elements.complimentary_pages_info_icon).to_be_visible(timeout=30000)
    self.elements.complimentary_pages_info_icon.hover()


def verify_additional_pages_value(self, expected_used, expected_total):
    """Verify additional pages value displays correct usage."""
    expect(self.elements.additional_pages_value).to_contain_text(f"{expected_used} of {expected_total}", timeout=30000)
